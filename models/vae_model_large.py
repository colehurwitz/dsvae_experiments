...
class VAE(nn.Module):
    def __init__(self, z_dim, input_shape, device=None):
        ...
        #DECODER
        self.fc2 = nn.Linear(z_dim, 512*int(self.input_height/16)*int(self.input_width/16))
        weights_init(self.fc2)
        self.instance_norm_d5 = nn.InstanceNorm2d(num_features=512, affine=False) 
        
        self.d5 = nn.ConvTranspose2d(512, 256, 4, stride=2, padding=1) #[b, 256,inh/8,inw/8]
        weights_init(self.d5)
        self.instance_norm_d6 = nn.InstanceNorm2d(num_features=256, affine=False) 
        
        self.d6 = nn.ConvTranspose2d(256, 128, 4, stride=2, padding=1) #[b, 128,inh/4,inw/4]
        weights_init(self.d6)
        self.instance_norm_d7 = nn.InstanceNorm2d(num_features=128, affine=False) 
        
        self.d7 = nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1) #[b, 64,inh/2,inw/2]
        weights_init(self.d7)
        self.instance_norm_d8 = nn.InstanceNorm2d(num_features=64, affine=False) 
        
        self.d8 = nn.ConvTranspose2d(64, self.input_nc, 4, stride=2, padding=1) #[b, innc,inh,inw]
    ...
    def decode(self, z):
        ...
        h = h.reshape(-1, 512, int(self.input_height/16), int(self.input_width/16))
